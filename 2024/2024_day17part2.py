...
#chemin = "datas/2024_day17_exemple2"
...
goal = program[::-1]
def findAtoGoal(A = 0, profondeur = 0):
    global dicRegisters
    if profondeur == len(goal): #on a finit de tout parcourir
        return A
    for i in range (8): #chiffres sur 3 bits
        dicRegisters["A"] = A * 8 + i
        output = parcoursOutput(program)
        if output and int(output[0]) == goal[profondeur]: #c'est la valeur recherchée
            tryNext = findAtoGoal(A * 8 + i, profondeur+1)
