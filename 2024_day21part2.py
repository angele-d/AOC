...
def allPaths(pointA,pointB,without):
    ...
    if distY <= 0: #pointB à gauche de pointA
        ...
        onePath = onePath + '>'*abs(distY)
    if len(onePath) in [0,1]: allPermutations = [onePath]
    elif onePath[0]=="v" and onePath[-1]==">": allPermutations = [onePath[::-1]]
    elif onePath[0]=="^" and onePath[-1]==">": allPermutations = [onePath[::-1]]
    else: allPermutations = [onePath]
...
def findShortest(allMove):
    minlen = (min(len(mv) for mv in allMove))
    returnMove = []
    for move in allMove:
...
def boucleSurDirectional(shortMove, acc = 0):
    print(acc)
    if acc == 24:
        allMoves = []
        for code in shortMove:
            allMoves = allMoves + moveDirectionalPad(code)
        return min(len(mv) for mv in allMoves)
    else:
        allMoves = []
        for code in shortMove:
            allMoves = allMoves + moveDirectionalPad(code)
        shortMove = findShortest(allMoves)
        return boucleSurDirectional(shortMove, acc+1)
...
def main(chemin):
    ...
    for code in fiveCodes:
        ...
        lenFiveCodes.append(boucleSurDirectional(shortMove1))
