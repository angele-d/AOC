...
def antinode_key(coords_key):
    ...
    for i in range (len(coords_key)-1):
        for j in range (i+1,len(coords_key)):
            coord1 = coords_key[i]
            coord2 = coords_key[j]
            if lignes[coord1[0]][coord1[1]] != "#":
                    nb_antinodes += 1
                    lignes[coord1[0]][coord1[1]] = "#"
            if lignes[coord2[0]][coord2[1]] != "#":
                    nb_antinodes += 1
                    lignes[coord2[0]][coord2[1]] = "#"
            ...
            while est_dans_tableau(hash1):
                if lignes[hash1[0]][hash1[1]] != "#":
                    nb_antinodes += 1
                    lignes[hash1[0]][hash1[1]] = "#"
                hash1 = (hash1[0]-x_diff,hash1[1]-y_diff)
            while est_dans_tableau(hash2):
                if lignes[hash2[0]][hash2[1]] != "#":
                    nb_antinodes += 1
                    lignes[hash2[0]][hash2[1]] = "#"
                hash2 = (hash2[0]+x_diff,hash2[1]+y_diff)
